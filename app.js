new Vue({
    ...
    methods: {
        ...
        special_attack: function () {
            ...
            this.monster_heal -= point;
            this.add_to_log({ turn: "p", text: this.logs_text.special_attack + point })
            this.monster_attack();
        },
        heal_up: function () {
            var point = Math.ceil(Math.random() * this.heal_up_multiple);
            this.monster_heal -= point;
            this.add_to_log({ turn: "p", text: this.logs_text.heal_up + point })
            this.monster_attack();
        },
        give_up: function () {
            this.player_heal = 0;
            this.add_to_log({ turn: "p", text: this.logs_text.give_up })
        },
        monster_attack: function () {
            var point = Math.ceil(Math.random() * this.monster_attack_multiple);
            this.player_heal -= point;
            this.add_to_log({ turn: "m", text: this.logs_text.monster_attack + point })
        },
